...
def products():
    ...
    if search:
        ...
        keyword_map = {
            'ao': [
                'ao', 'thun', 'hoodie', 'somi', 'so mi',
                'khoac', 'len', 'body', 'cardigan', 'blazer'
            ],
            'aothun': ['ao', 'thun'],
            'ao thun': ['ao', 'thun'],
            'aokhoac': ['ao', 'khoac', 'hoodie'],
            'ao khoac': ['ao', 'khoac', 'hoodie'],
            'aosomi': ['ao', 'somi', 'so mi'],
            'ao so mi': ['ao', 'somi', 'so mi'],
            'quan': [
                'quan', 'jean', 'jeans', 'kaki',
                'short', 'tay', 'jogger'
            ],
            'quanjean': ['quan', 'jean', 'jeans'],
            'quan jean': ['quan', 'jean', 'jeans'],
            'quanjeans': ['quan', 'jean', 'jeans'],
            'quan jeans': ['quan', 'jean', 'jeans'],
            'vay': ['vay', 'dam', 'chan vay'],
            'giay': ['giay', 'sneaker', 'boot', 'dep', 'sandal'],
            'non': ['non', 'mu', 'cap'],
            'mu': ['non', 'mu', 'cap'],
        ...
        related_keywords = keyword_map.get(
            search_norm,
            [search_norm]
        )
...
def product_detail(pid):
    ...
    admin_redirect = redirect_admin_to_dashboard()
    if admin_redirect:
...
def admin_statistics():
    ...
    for user in users:
        ...
        for order in user_orders:
            ...
            for i in range(len(names)):
                for j in range(i + 1, len(names)):
                    combo_counter[f"{names[i]} + {names[j]}"] += 1
    ...
    if transactions:
        ...
        try:
            ...
            print("Apriori statistics error:", e)
...
def admin_statistics():
    ...
    'admin/statistics.html',
    apriori_rules=apriori_rules,
    chart_exists=len(chart_labels) > 0
)
